...
from app.schemas.cart_schema import ItemInCartResponse, to_item_in_cart_response
...
@router.get(
    "/", response_model=list[ItemInCartResponse], status_code=status.HTTP_200_OK
)
def get_cart(
    current_user: User = Depends(get_current_user),
    cart_item_repo: CartItemRepository = Depends(CartItemRepository),
):
    itens_in_cart = cart_item_repo.get_cart(current_user.id)

    return [to_item_in_cart_response(item_in_cart) for item_in_cart in itens_in_cart]
